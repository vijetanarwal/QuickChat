package com.vijet.quickchat.repository
...
class MainRepository @Inject constructor(
    ...
    suspend fun updateOnlineStatus(userId: String, isOnline: Boolean) {
        try {
            val data = hashMapOf<String, Any>(
                Constant.KEY_IS_ONLINE to isOnline,
                Constant.KEY_LAST_SEEN to System.currentTimeMillis()
            )
    ...
    suspend fun updateProfileImage(userId: String, encodedImage: String) {
        try {
            fireStore.collection(Constant.KEY_COLLECTION_USERS)
                .document(userId)
                .update(Constant.KEY_IMAGE, encodedImage)
                .await()
        } catch (e: Exception) { }
    }
