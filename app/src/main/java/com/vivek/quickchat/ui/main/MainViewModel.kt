...
import com.vijet.quickchat.utils.clearAll
import com.vijet.quickchat.utils.putAny
...
class MainViewModel @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            pref.getString(Constant.KEY_USER_ID, null)?.let {
                try {
                    repository.updateToken(repository.getToken(), it)
                    repository.updateOnlineStatus(it, true)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
    ...
    fun loadUserDetails() = pref.getString(Constant.KEY_IMAGE, "") ?: ""
    ...
    fun signOut(): LiveData<Boolean> {
        ...
        viewModelScope.launch {
            ...
            userData[Constant.KEY_IS_ONLINE] = false
            userData[Constant.KEY_LAST_SEEN] = System.currentTimeMillis()
            val isSignOut =
    ...
    fun updateProfileImage(encodedImage: String) {
        viewModelScope.launch {
            pref.getString(Constant.KEY_USER_ID, null)?.let { userId ->
                repository.updateProfileImage(userId, encodedImage)
                pref.putAny(Constant.KEY_IMAGE, encodedImage)
            }
        }
    }
